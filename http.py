...
def flood(target):
    ...
    try:
        ...
        print(f"{Fore.RED}[!] {Fore.BLUE}Timed out{Fore.RESET}")
    except Exception as e:
        print(
            f"{Fore.BLUE}Error sending requests GET\n{Fore.BLUE}{e}{Fore.RESET}"
        )
    else:
        print(
            f"{Fore.GREEN}╚══{r.status_code}╬═══{Fore.BLUE}╣RequestSendingToWeb╠═══╝ {len(payload)}.{Fore.RESET}"
        )
