...
def GetMethodByName(method):
    ...
    if method in ("HTTP", "SLOWLORIS"):
        ...
        raise SystemExit(
            f"{Fore.RED}[!] {Fore.BLUE}Unknown DDOS method... {repr(method)} selected..{Fore.RESET}"
        )
...
class AttackMethod:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        print(f"{Fore.BLUE}[!] {Fore.GREEN}Attack Completed!{Fore.RESET}")
    ...
    def Start(self):
        ...
        print(
            f"{Fore.RED}[?] {Fore.GREEN}Starting the attack {target} using the method {self.name}.{Fore.RESET}\n"
            f"{Fore.RED}[?] {Fore.GREEN}The attack will stop after {Fore.GREEN}{duration}{Fore.BLUE}.{Fore.RESET}"
        )
        ...
        try:
            ...
            self.is_running = False
            print(
                f"\n{Fore.RED}[!] {Fore.BLIE}Ctrl+C detected. Stopping {self.threads_count} threads..{Fore.RESET}"
            )
