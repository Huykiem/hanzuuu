...
def create_socket(target):
    try:
        ...
        print(f"{Fore.RED}[-] {Fore.BLUE}Time's up...{Fore.RESET}")
    except socket.error:
        print(f"{Fore.RED}[-] {Fore.BLUE}There was an error creating socket{Fore.RESET}")
    else:
    ...
    for _ in range(4):
        for index, sock in enumerate(sockets):
            try:
                ...
                print(
                    f"{Fore.RED}[-] {Fore.BLUE}There was an error sending headers keep-alive{Fore.RESET}"
                )
                sockets.remove(sock)
            else:
                print(
